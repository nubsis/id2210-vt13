...
public final class Search extends ComponentDefinition {
            ...
            synchronized (sync) {
                ...
                try {
                    if (args[0].compareToIgnoreCase("search") == 0) {
                        response = searchPageHtml(args[1]);
    ...
    List<IndexEntry> getEntries(LinkedList<Integer> ids) {
        ...
        try {
            IndexSearcher searcher = null;
            IndexReader reader = null;
            ScoreDoc[] hits = getExistingDocsInRange(
                    ids.getFirst(),
                    Math.min(ids.getLast(), maxIndexEntry),
                    reader,
                    searcher);
            if (hits != null) {
        ...
        public void handle(Request event) {
            ...
            synchronized (sync) {
                ...
                for (Address address : gradientBelow) {
                    trigger(new Push.Offer(self, address, event.getEntryId(), event.getId()), networkPort);
                }
            }
        }
    };
    Handler<Insert.Response> handlerInsertResponse = new Handler<Insert.Response>() {
        ...
        public void handle(Insert.Response event) {
            synchronized (sync) {
                Request request = requests.get(event.getRequestId());
                if (request == null) {
                    return;
                }
                if (event.isSuccess()) {
