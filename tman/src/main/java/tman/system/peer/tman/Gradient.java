...
public class Gradient {
	...
	public int compare(Address a1, Address a2) {
	    return Integer.compare(a1.getId(), a2.getId())
		    * (type == CompareType.LowToHigh ? 1 : -1);
	}
    }
    ...
    // -------------------------------------------------------------------
    private final Handler<CyclonSample> handlerCyclonSample = new Handler<CyclonSample>() {
	@Override
	public void handle(CyclonSample event) {
	    merge(event.getSample());
	}
    };
    private final Handler<NeighbourExchange.Request> handlerExchangeRequest = new Handler<NeighbourExchange.Request>() {
	@Override
	public void handle(NeighbourExchange.Request event) {
	    tman.send(new NeighbourExchange.Response(tman.getSelf(), event
		    .getSource(), getAll()));
	}
    };
    private final Handler<NeighbourExchange.Response> handlerExchangeResponse = new Handler<NeighbourExchange.Response>() {
	@Override
	...
	for (Address a : newPartners) {
	    if (a == null
		    || a.getId() % TManConfiguration.PARTITION_COUNT != tman
		    .getPartitionId() || higher.contains(a)
		    || lower.contains(a)) {
		continue;
	    }
	...
	Collections.sort(higher, new GradientComparator(
		GradientComparator.CompareType.LowToHigh));
	Collections.sort(lower, new GradientComparator(
		GradientComparator.CompareType.HighToLow));
	...
	tman.send(new NeighbourExchange.Request(tman.getSelf(),
		new LinkedList<>(collection).get(random.nextInt(collection
			.size()))));
    }
